...
with col1:
    render_metric_card("Total Models", str(len(DISEASES)), "🧠", COLORS["primary"])
with col2:
    render_metric_card("Avg Accuracy", f"{avg_acc*100:.1f}%", "🎯", COLORS["success"])
with col3:
    render_metric_card("Avg F1 Score", f"{avg_f1:.3f}", "⚖️", COLORS["warning"])
with col4:
    render_metric_card("Clinical Samples Trained", "8,500+", "📈", COLORS["secondary"])
...
with col_chart1:
    ...
    try:
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                title=dict(text="Accuracy (%)", font=dict(color='#CBD5E1')),
                range=[0, 105],
                gridcolor="#334155"
            ),
